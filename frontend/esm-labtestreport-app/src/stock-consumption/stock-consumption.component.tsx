...
export default function StockConsumptionReport() {
  ...
  const { rows: rawRows, isLoading } = useStockConsumptionReport(
    primaryStartDate,
    primaryEndDate,
    locationUuid || undefined,
  );
  const { rows: compareRawRows, isLoading: compareLoading } = useStockConsumptionReport(
  ...
  const kpiItems = useMemo(() => {
    const totalConsumed = rows.reduce((sum, row) => sum + row.quantity, 0);
    const distinctItems = new Set(rows.map((row) => row.stockItemId)).size;
    const items_ = [
      { label: t('itemsWithActivity', 'Items With Activity'), value: distinctItems },
      { label: t('totalConsumed', 'Total Quantity Consumed'), value: totalConsumed },
    ];
    if (!compare.enabled) {
      return items_;
    }
  ...
  // Sorted so the item whose consumption grew the most (in absolute terms) sorts first -
  // directly answers "which item's consumption has increased" rather than just listing items.
  ...
  const mainExportSheet = useMemo<ExportSheet>(
    () => ({
      name: t('stockConsumption', 'Stock Consumption'),
      headers: [
        t('item', 'Item'),
    ...
    <div>
      ...
      <div className={pageStyles.pageBody}>
        <h2 className={pageStyles.pageHeading}>
          {t('stockConsumptionReportTitle', 'Stock Consumption by Location Report')}
        </h2>
        ...
        {!dataLoading && compare.enabled && (
          <>
            <h3 className={pageStyles.pageHeading}>
              {t('consumptionComparisonHeading', 'Item consumption comparison: {{primary}} vs {{comparison}}', {
        ...
        <ExportButtons
          filenameBase="stock-consumption-report"
            ...
            <table className={pageStyles.dataTable}>
              <thead>
                <tr>
                  ...
                  <SortableHeader
                    label={t('quantityConsumed', 'Quantity Consumed')}
                    sortKey="quantity"
                    activeSortKey={sortKey}
                    direction={direction}
                    onSort={toggleSort}
