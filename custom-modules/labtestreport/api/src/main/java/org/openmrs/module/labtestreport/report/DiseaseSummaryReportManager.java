...
/**
 * Registers the disease surveillance summary report with the Reporting module so it also shows
 * up in the O3 Reports dashboard (as a plain data table, without the admin page's clickable rows).
 */
public class DiseaseSummaryReportManager extends BaseReportManager {

	public static final String UUID = "8a2e4f60-3b1c-4d7e-9a5f-6c0b8e2d1a70";
	...
	public String getName() {
		return "Disease Surveillance Summary Report";
	}

	@Override
	public String getDescription() {
		return "Diagnoses broken down by category, age group and gender. For clickable drill-down to the patients "
		        + "behind each count, use the report under Administration instead.";
	}
	...
	public ReportDefinition constructReportDefinition() {
		...
		reportDefinition.addDataSetDefinition("diseaseSummary", Mapped.mapStraightThrough(dataSetDefinition));
	...
	private static String buildPreviewSql() {
		return "SELECT category AS `Category`, diagnosisLabel AS `Diagnosis`, totalCases AS `Total Cases`, "
		        ...
		        + "total AS `Total` "
		        + "FROM (" + SqlResources.load("disease_summary_report.sql") + ") base";
	}
}
